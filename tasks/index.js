...
.setAction(async function ({ token, spender, deadline }, { ethers: { getNamedSigner } }, runSuper) {
  const erc20 = await ethers.getContractFactory("TacoswapV2ERC20")
...
.setAction(async function ({ tokenA, tokenB, tokenADesired, tokenBDesired, tokenAMinimum, tokenBMinimum, to, deadline }, { ethers: { getNamedSigner } }, runSuper) {
  const router = await ethers.getContract("TacoswapV2Router")
...
task("migrate", "Migrates liquidity from Tacoswap to TacoSwap")
...
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  ...
  const bar = await ethers.getContract("TacoBar")

  await run("erc20:approve", { token: taco.address, spender: bar.address })
  
  await (await bar.connect(await getNamedSigner("dev")).enter(amount)).wait()
});

task("bar:leave", "TacoBar leave")
.addParam("amount", "Amount")
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  const taco = await ethers.getContract("TacoToken")
...
task("maker:serve", "TacoBar serve")
.addParam("a", "Token A")
.addParam("b", "Token B")
